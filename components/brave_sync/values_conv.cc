...
namespace {
  ...
  std::unique_ptr<base::Value> BytesDataFromString(const std::string &data_string) {
    // data_string: comma-separated 0..255 integers
    // result:
/*           TYPE=DICTIONARY
             [
                name=0
                TYPE=INTEGER VALUE=157
                name=1
...
*/
    ...
    for (size_t i = 0; i < splitted.size(); ++i) {
      ...
      CHECK(b);
      value->SetKey(base::NumberToString(i), base::Value(output));
    }
  ...
  */
  ...
  value->SetKey("deviceId", std::move(*BytesDataFromString(device_id)));
  value->SetKey("objectId", std::move(*BytesDataFromString(object_id)));
...
std::unique_ptr<base::Value> CreateBookmarkSyncRecordValue(
  ...
  bool hideInToolbar,
  const std::string &order) {
  //std::unique_ptr<base::Value> sync_record_value = std::make_unique<base::Value>(base::Value::Type::DICTIONARY);
...
// std::string GetObjectId(const base::Value &sync_record) {
//   DCHECK(sync_record.is_dict());
//   if (!sync_record.is_dict()) {
//     return "";
//   }
//
//   // const base::Value* val_object_id = sync_record.FindKey("objectId");
//   // DCHECK(val_object_id != nullptr);
//   // if (val_object_id == nullptr) {
//   //   return "";
//   // }
//   // DCHECK(val_object_id->is_dict());
//   // if (!val_object_id->is_dict()) {
//   //   return "";
//   // }
// /*
// name=objectId
...
// name 0...15
// */
//   std::string object_id = ExtractObjectId(sync_record);
//   return object_id;
// }
