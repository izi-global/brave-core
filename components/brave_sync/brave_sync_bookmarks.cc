...
std::unique_ptr<base::Value> BraveSyncBookmarks::GetResolvedBookmarkValue(
  ...
  */
  ...
  //#include "components/bookmarks/browser/bookmark_model.h"//BookmarkModel* model
  //#include "components/bookmarks/browser/bookmark_utils.h"//const BookmarkNode* GetBookmarkNodeByID(const BookmarkModel* model, int64_t id);
...
std::unique_ptr<base::Value> BraveSyncBookmarks::BookmarkToValue(const bookmarks::BookmarkNode* node,
  ...
  std::unique_ptr<base::Value> value = CreateBookmarkSyncRecordValue(
    jslib_const::kActionCreate, // kActionCreate/kActionUpdate/kActionDelete 0/1/2
    ...
    parent_folder_object_sync_id,//ToObjectId(node->parent() ? node->parent().id()), //const std::string &parentFolderObjectId,
